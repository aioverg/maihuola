<template>
	<view>
		...
		<view class="earn-page-body">
			...
			<!--收益来源分类，卖货、赚金-->
			<view class="es-kind">
				<view class="es-kind-item" v-for="(item, index) in esKind" @click="selKind(item.id)" :key="index" :class="item.id == elKind ? 'es-sel-item' : '' ">
					<view>{{item.name}}</view>
			...
			<!--今日收益、昨日收益、近七日收益 菜单-->
			<view class="day-earn">
				<view class="de-item" v-for="(item, index) in dayMenu"  @click="selMenu(item.id)" :class="item.id == elMenu ? 'de-sel-item' : '' " :key="item.id">{{item.name}}</view>
			</view>
			<!--今日收益、昨日收益、近七日收益 内容-->
			<view class="earn-content">
				<view class="ec-one">
			...
			<view class="earn-detail">
				<view class="earn-detail-item" @click="navTo('/pages/userinfo/earnDetail')">
					<ai-list-cell title="收益结算明细"></ai-list-cell>
				</view>
			</view>
		</view>
	</view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				esKind: [{id: "0", name: "卖货"}, {id: "1", name: "赚金"}],
				elKind: "0",
				dayMenu: [{id: "0", name: "今日"},{id: "1", name: "昨日"},{id: "2", name: "近七日"}],
				elMenu: "0"
			}
		},
		methods: {
