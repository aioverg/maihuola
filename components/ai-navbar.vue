	...
	<view class="ai-navbar" >
		...
		<view
			...
			<block v-if="backgroundImg">
				<image class="navbgimg" :src="backgroundImg"></image>
			</block>
			
			<view :style="{ height: statusBarHeight ,'background': statusBarBackground}" class="hx-status-bar" v-if="statusBar" ></view>
			<view :style="{color:colorInfo,height: height,'line-height':height}" class="hd ai-navbar__header ai-navbar__content_view">
				<view class="ai-navbar__header-btns ai-navbar__content_view"  @tap="onClickLeft" v-if="leftSlot" :style="{'color': colorInfo}">
					<block v-if="leftText.length || leftIcon.length || back">
						<view 
							v-if="leftIcon.length || back"
							:class="back ? 'left_back' : ''"
							class="ai-navbar__content_view" >
							<uni-icons v-if="leftArrow" :type="back ? 'arrowleft' : leftIcon" :color="colorInfo" size="22"/>
						</view>
						<view
							v-if="leftText.length"
							:class="{'ai-navbar-btn-icon-left':!leftIcon.length}"
							class="ai-navbar-btn-text ai-navbar__content_view">
							{{ leftText }}
						</view>
				...
				<view class="ai-navbar__header-container ai-navbar__content_view" style="overflow: hidden;">
					<view
					  v-if="title.length"
					  class="ai-navbar__header-container-inner ai-navbar__content_view" style="display: inline-block;">{{ title }}</view>
					<!-- 标题插槽 -->
				...
				<view :class="title.length?'ai-navbar__header-btns-right':''"
					class="ai-navbar__header-btns ai-navbar__content_view"
					@tap="onClickRight"
					v-if="rightSlot">
					<!-- 优先显示图标 -->
					<block v-if="rightIcon.length || rightText.length">
						...
						<view v-if="rightText.length" class="ai-navbar-btn-text ai-navbar__content_view">{{ rightText }}</view>
					</block>
					<block v-else>
			...
			transparentValue(val,oldVal) {
				...
				if (tt.setNavigationBarColor) {
				 if(oldVal > 0.8){
				  ...
				  console.log("ai-navbar 提示：当前客户端版本过低，无法使用状态栏颜色修改功能，请升级（基础库1.40+）。")
				}
				// #endif
...
<style lang="scss">
	...
	//防止其他ui影响
	.ai-navbar uni-view,
	.ai-navbar uni-scroll-view,
	.ai-navbar uni-swiper,
	.ai-navbar uni-button,
	.ai-navbar uni-input,
	.ai-navbar uni-textarea,
	.ai-navbar uni-label,
	.ai-navbar uni-navigator,
	.ai-navbar uni-image {
		box-sizing: unset;
	}
	.ai-navbar {
